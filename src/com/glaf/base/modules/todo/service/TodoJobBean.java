...
public class TodoJobBean {
	...
	public Map getMMToDoMap() {
		return todoService.getMMToDoMap();
	}
				...
				if (StringUtils.isNotEmpty(todo.getSql())) {
					...
					if (rowsMap.size() > 0) {
						...
						while (iter.hasNext()) {
							...
							Date limitWorkDate = todoService
									.getWorkCalendarService().getWorkDate(
											startDate, limitDay);
		...
		if (taskItems != null && taskItems.size() > 0) {
			...
			while (iterator.hasNext()) {
				...
				if (todo != null) {
					...
					Date limitWorkDate = todoService.getWorkCalendarService()
							.getWorkDate2(model.getStartDate(), limitDay);
					long time = limitWorkDate.getTime();
	...
	public void createTasksFromSQL() {
		...
		todoService.createTasksFromSQL(rows);
	}
	...
	public void createTasksFromWorkflow() {
		...
		todoService.createTasksFromWorkflow(tasks);
	}

	public void createTasksFromWorkflow(String actorId) {
		List tasks = this.getJbpmTasks(actorId);
		todoService.createTasksFromWorkflow(actorId, tasks);
	}
	...
	public List getTodoInstances(String actorId) {
		...
		bean.setSysUserService(todoService.getSysUserService());
	...
	public Collection getAllUsers() {
		SysUserService sysUserService = todoService.getSysUserService();
