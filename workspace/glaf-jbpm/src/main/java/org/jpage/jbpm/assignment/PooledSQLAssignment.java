 ...
 * 
 */
public class PooledSQLAssignment implements AssignmentHandler {
	private static final Log logger = LogFactory
			.getLog(PooledSQLAssignment.class);
	...
	public void assign(Assignable assignable, ExecutionContext ctx) {
		logger.debug("-------------------------------------------------------");
		logger.debug("----------------PooledSQLAssignment--------------------");
		logger.debug("-------------------------------------------------------");
		...
		if (task != null) {
			...
			if (StringUtils.isNotBlank(sql)) {
				...
				if (elements != null) {
					Map dataMap = (Map) CustomFieldInstantiator.getValue(
							Map.class, elements);
