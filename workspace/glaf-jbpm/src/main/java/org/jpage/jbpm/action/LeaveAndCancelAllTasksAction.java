...
public class LeaveAndCancelAllTasksAction implements ActionHandler {
	private static final Log logger = LogFactory
			.getLog(LeaveAndCancelAllTasksAction.class);
	...
	public void execute(ExecutionContext ctx) throws Exception {
		logger.debug("--------------------------------------------------------");
		logger.debug("-----------LeaveAndCancelAllTasksAction-----------------");
		logger.debug("--------------------------------------------------------");
		...
		if (CacheFactory.get("leave_and_cancel_" + processInstance.getId()) != null) {
			return;
		}
				...
				if (unfinishedTasks.size() > 0) {
					...
					while (iter.hasNext()) {
						...
						if (StringUtils.isNotBlank(taskNames)) {
							if (taskNames.indexOf(taskInstance.getName()) == -1) {
								continue;
							}
						}
