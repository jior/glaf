...
ttTree.prototype.drawNodes = function(parentNodeDiv, node, depth){
  ...
  for(var i=0; i< node.childNodes.length; i++){//遍历子节点
	...
	input.type="checkbox";
	input.id="cb_"+childNode.id;
	//input.name="cb_departId";
	input.border = "0";
	input.value="{id:"+childNode.nodeId+",name:'"+childNode.name+"'}";
...
ttTree.prototype.getNodes = function(node, depth){  
  ...
  if(xmlhttp){
    ...
    xmlhttp.open("GET", context+"/sys/department.do?method=getSubDeptAll&id=" + node.id , true);
    xmlhttp.send(null);
  }
}
/**
