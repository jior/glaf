...
<script type="text/javascript">
   ...
   jQuery(function(){
	...
	function addNew(){
	    var link="<%=request.getContextPath()%>/mx/dts/dataTransfer/edit";
	    art.dialog.open(link, { height: 420, width: 780, title: "添加记录", lock: true, scrollbars:"no" }, false);
	}

	function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/dts/dataTransfer/edit?id='+row.id;
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = "<%=request.getContextPath()%>/mx/dts/dataTransfer/edit?id="+selected.id;
		  art.dialog.open(link, { height: 420, width: 780, title: "修改记录", lock: true, scrollbars:"no" }, false);
	    }
	}
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="<%=request.getContextPath()%>/mx/dts/dataTransfer/edit?readonly=true&id="+selected.id;
		}
	}
