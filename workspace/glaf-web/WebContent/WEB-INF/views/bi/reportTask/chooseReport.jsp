...
<%
		...
		List unselecteds = (List)request.getAttribute("unselecteds");
		List selecteds = (List)request.getAttribute("selecteds");
        if(unselecteds != null && unselecteds.size() > 0){
		    for(int j=0;j<unselecteds.size();j++){
				Report m = (Report) unselecteds.get(j);
				if(selecteds != null && selecteds.contains(m.getId())){
                    buffery.append("\n<option value=\"").append(m.getId()).append("\">")
						   .append(m.getId()).append(" [").append(m.getSubject()).append("]")
						   .append("</option>");
				} else {
				    bufferx.append("\n<option value=\"").append(m.getId()).append("\">")
						   .append(m.getId()).append(" [").append(m.getSubject()).append("]")
...
<table class="table-border" align="center" cellpadding="4"
	...
	<tbody>		 
		<tr>
			...
			<div align="center">
			   <div align="center">可选报表</div>
			</div>
			</td>
			<td></td>
			<td colspan="2" width="212">
			<div align="center">已选报表</div>
			</td>
		</tr>
		<tr>
			<td width="2">&nbsp;</td>
