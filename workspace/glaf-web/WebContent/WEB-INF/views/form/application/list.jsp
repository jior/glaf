...
<script type="text/javascript">
   ...
   var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/form/application.do?method=json&nodeId='+treeNode.id);
	}
    ...
    jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/form/application.do?method=json',
				sortName: 'id',
				sortOrder: 'desc',
				remoteSort: false,
				singleSelect:true,
	...
	function addNew(){
	    //location.href="<%=request.getContextPath()%>/form/application.do?method=edit";
		var nodeId = jQuery("#nodeId").val();
		var link="<%=request.getContextPath()%>/form/application.do?method=edit&nodeId="+nodeId;
	    art.dialog.open(link, { height: 420, width: 680, title: "添加记录", lock: true, scrollbars:"no" }, false);
	}

	function onRowClick(rowIndex, row){
            //window.open('<%=request.getContextPath()%>/form/application.do?method=edit&appId='+row.id);
	    var link = '<%=request.getContextPath()%>/form/application.do?method=edit&appId='+row.id;
	...
	function editSelected(){
	    ...
	    if (selected ){
		//location.href="<%=request.getContextPath()%>/form/application.do?method=edit&appId="+selected.id;
		var link = "<%=request.getContextPath()%>/form/application.do?method=edit&appId="+selected.id;
		art.dialog.open(link, { height: 420, width: 680, title: "修改记录", lock: true, scrollbars:"no" }, false);
	    }
	}
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="<%=request.getContextPath()%>/form/application.do?method=edit&readonly=true&appId="+selected.id;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/form/application.do?method=delete&appIds='+appIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
