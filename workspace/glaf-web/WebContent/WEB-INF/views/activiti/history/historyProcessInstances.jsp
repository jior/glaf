...
<br>
 ...
 <script type="text/javascript"> 
  ...
  YAHOO.example.DynamicData = ( function() {
    ...
    var s_link = "<%=request.getContextPath()%>/mx/activiti/history/historyProcessInstanceJson?q=1"+x_app;
	...
	function onMyDblClickRow(oArgs) {
	 ...
	 var formatFunKey = function(elCell, oRecord, oColumn, oData) {
	  ...
	  var formatState = function(elCell, oRecord, oColumn, oData) {
        var state = YAHOO.lang.dump(oRecord.getData("state"));
    	var msg = "";
        if (state == "active") {
            msg = "<span style='color: blue; font-weight: bold; text-align: center;'> 运行中 </span>";
    	}  else {
            msg = "<span style='color: green; font-weight: bold; text-align: center;'> 已完成  </span>";
    	}
        elCell.innerHTML = msg;
      }

	 var formatEndDate = function(elCell, oRecord, oColumn, oData) {
        var endDate = YAHOO.lang.dump(oRecord.getData("endDate"));
    	var msg = "";
        if (endDate != null) {
            msg = "<span style='color: green; font-weight: bold; text-align: center;'>"+endDate+"</span>";
    	}   
        elCell.innerHTML = msg;
      }
	  ...
	  YAHOO.widget.DataTable.Formatter.formatState = formatState;
