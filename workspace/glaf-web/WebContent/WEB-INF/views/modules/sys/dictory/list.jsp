...
<script type="text/javascript">
    ...
    var setting = {
			async: {
				enable: true,
				url:"<%=request.getContextPath()%>/rs/tree/treeJson?nodeCode=011",
				dataFilter: filter
			},
			callback: {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/sys/dictory.do?method=json&nodeId='+treeNode.id);
	}
   ...
   jQuery(function(){
	...
	function addNew(){
		...
		var link = "<%=request.getContextPath()%>/sys/dictory.do?method=prepareAdd&parent="+nodeId;
	    art.dialog.open(link, { height: 400, width: 480, title: "添加字典", lock: true, scrollbars:"yes" }, false);
	...
	function onRowClick(rowIndex, row){
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/sys/dictory.do?method=prepareModify&id='+row.id;
	    art.dialog.open(link, { height: 400, width: 480, title: "修改字典", lock: true, scrollbars:"yes" }, false);
		//openWindow(link, 400, 480, "yes");
	}
	...
	function editSelected(){
	    ...
	    if (selected ){
		  //location.href="<%=request.getContextPath()%>/sys/dictory.do?method=edit&rowId="+selected.id;
		  var link = "<%=request.getContextPath()%>/sys/dictory.do?method=prepareModify&id="+selected.id;
		  art.dialog.open(link, { height: 400, width: 480, title: "修改字典", lock: true, scrollbars:"yes" }, false);
		  //openWindow(link, 400, 480, "yes");
	    }
	}
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="<%=request.getContextPath()%>/sys/dictory.do?method=prepareModify&id="+selected.id;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/sys/dictory.do?method=delete&rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
