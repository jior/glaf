...
<script type="text/javascript">
    ...
    var setting = {
			async: {
				enable: true,
				url:"<%=request.getContextPath()%>/rs/sys/application/treeJson?nodeCode=02",
				dataFilter: filter
			},
			callback: {
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/sys/application.do?method=json',
				sortName: 'id',
				sortOrder: 'desc',
				remoteSort: false,
				singleSelect:true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/sys/application.do?method=delete&rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
