...
<script type="text/javascript">
   ...
   var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadMxData('<%=request.getContextPath()%>/mx/sys/template/json?nodeId='+treeNode.id);
	}
	...
	function reLoadData(nodeId){
		loadData('<%=request.getContextPath()%>/mx/sys/template/json?nodeId='+nodeId);
	}
	...
	function addNew(){
	    ...
	    art.dialog.open(link, { height: 480, width: 600, title: "添加记录", lock: true, scrollbars:"no" }, false);
		//location.href=link;
	}

	function onRowClick(rowIndex, row){
		var nodeId = jQuery("#nodeId").val();
        //window.open('<%=request.getContextPath()%>/mx/sys/template/edit?id='+row.id);
	    var link = '<%=request.getContextPath()%>/mx/sys/template/edit?templateId='+row.id+"&nodeId="+nodeId;
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/template/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
