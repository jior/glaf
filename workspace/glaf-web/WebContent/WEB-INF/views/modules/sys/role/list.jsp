...
<script type="text/javascript">
   ...
   jQuery(function(){
	...
	function onMyRowClick(rowIndex, row){
		...
		jQuery('#editForm').form('load', '<%=request.getContextPath()%>/rs/sys/role/detail?id='+row.id);
	}
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/sys/role/'+res,
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   //alert('服务器处理错误！');
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/sys/role/batchDelete?rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   if(data != null && data.message != null){
						   //alert(data.message);
    ...
    <form id="editForm" name="editForm" method="post">
         ...
         <table class="easyui-form" width="95%" align="center" border="0" cellspacing="0" cellpadding="5">
		  <tr>
			...
			<td><input id="name" name="name" type="text" size="35" class="easyui-validatebox" datatype="string" nullable="no" maxsize="20" chname="角色名称" data-options="required:true"></td>
		  </tr>
		  <tr>
			<td width="20%" align="left" class="input-box2" valign="top">代码*</td>
			<td><input id="code" name="code" type="text" size="35" class="easyui-validatebox" datatype="string" nullable="no" maxsize="20" chname="角色代码" data-options="required:true"></td>
		  </tr>
		  <tr>
		 ...
		 <tr>
          ...
          字段选择&nbsp;&nbsp;
			  <select id="field" name="field" onchange="javascript:displayField();" >
				...
				<option value="code">角色代码</option>
				<option value="desc">角色描述</option>
			  </select>
