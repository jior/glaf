   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				columns:[[
					...
					{title:'群组名称',field:'name', width:120},
	...
	function onMyRowClick(rowIndex, row){
		...
		var url="group.do?method=prepareModify&type=${type}&groupId="+row.id;
	    var width=450;
	    var height=280;
	    var scroll="no";
	    //openWindow(url, width, height, scroll);
		jQuery('#edit_dlg').dialog('open').dialog('setTitle','群组编辑');
		jQuery('#editForm').form('load', '<%=request.getContextPath()%>/rs/base/group/detail?type=${type}&groupId='+row.groupId);
	}

	function searchWin(){
		jQuery('#dlg').dialog('open').dialog('setTitle','群组查询');
	...
	function groupUser(){
		...
		if (selected ){
		  var link = "<%=request.getContextPath()%>/base/group.do?method=groupUsers&groupId="+selected.groupId;
		  var width=450;
		  var height=480;
		  var scroll="yes";
	...
	function editSelected(){
		...
		if (selected ){
			 jQuery('#edit_dlg').dialog('open').dialog('setTitle','群组编辑');
			 jQuery('#editForm').form('load', '<%=request.getContextPath()%>/rs/base/group/detail?type=${type}&groupId='+selected.groupId);
		}
	}
	...
	function editSelected2(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
			var url="group.do?method=prepareModify&type=${type}&groupId="+selected.groupId;
			var width=450;
			var height=280;
			var scroll="no";
			openWindow(url, width, height, scroll);
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/base/group/'+res,
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   //alert('服务器处理错误！');
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/base/group/batchDelete?groupIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   if(data != null && data.message != null){
						   //alert(data.message);
...
<div id="edit_dlg" class="easyui-dialog" style="width:480px;height:280px;padding:10px 20px"
    ...
    <form id="editForm" name="editForm" method="post">
         ...
         <table class="easyui-form" width="95%" align="center" border="0" cellspacing="0" cellpadding="5">
		  <tr>
			<td width="20%" align="left" class="input-box">群组名称*</td>
			<td><input id="name" name="name" type="text" size="35" class="easyui-validatebox" datatype="string" nullable="no" maxsize="20" chname="群组名称" data-options="required:true"></td>
		  </tr>
		  <tr>
			<td width="20%" align="left" class="input-box2" valign="top">代码</td>
			<td><input id="code" name="code" type="text" size="35" class="easyui-validatebox" datatype="string" nullable="yes" maxsize="20" chname="群组代码" data-options="required:false"></td>
		  </tr>
		  <tr>
			<td width="20%" align="left" class="input-box2" valign="top">描　　述</td>
			<td><textarea id="desc" name="desc" cols="28" rows="5" class="easyui-textbox" datatype="string" nullable="yes" maxsize="100" chname="群组描述"></textarea>        
			</td>
		  </tr>
		  <tr>
		 ...
		 <tr>
          ...
          字段选择&nbsp;&nbsp;
			  <select id="field" name="field" onchange="javascript:displayField();" >
				...
				<option value="name">群组名称</option>
				<option value="code">群组代码</option>
				<option value="desc">群组描述</option>
			  </select>
