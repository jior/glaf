...
var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/branch/user.do?method=json&nodeId='+treeNode.id);
	}
   ...
   jQuery(function(){
	...
	function addNew(){
	    ...
	    var link="<%=request.getContextPath()%>/branch/user.do?method=prepareAdd&nodeId="+nodeId+"&deptId=${deptId}";
	...
	function onRowClick(rowIndex, row){
		...
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/branch/user.do?method=prepareModify&nodeId='+nodeId+'&deptId=${deptId}&id='+row.id;
