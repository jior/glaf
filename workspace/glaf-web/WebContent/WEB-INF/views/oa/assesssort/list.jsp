...
<script type="text/javascript">
   ...
   jQuery(function(){
	...
	function onRowClick(rowIndex, row){
	    ...
	    var link = '${contextPath}/mx/oa/assesscontent/edit?contentid='+row.id;
	...
	function editSelected(){
		...
		var link = "${contextPath}/mx/oa/assesscontent/edit?contentid="+selected.id;
		art.dialog.open(link, { height: 420, width: 680, title: "修改记录", lock: true, scrollbars:"no" }, false);
	    }
	}
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="${contextPath}/mx/oa/assesscontent/edit?readonly=true&contentid="+selected.id;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/oa/assesscontent/delete?contentids='+contentids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
