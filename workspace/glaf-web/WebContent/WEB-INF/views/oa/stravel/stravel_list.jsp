...
<%=request.getParameter("status")%>
	...
	function addNew() {
		var link = "${contextPath}/mx/oa/stravel/addOrEdit";
		art.dialog.open(link, {
			height : 420,
			width : 1050,
			title : "添加记录",
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '出差申请列表查询');
						...
						success : function(data) {
							if (data != null) {
								if (data.status == "0" || data.status == "3") {
									var link = "${contextPath}/mx/oa/stravel/addOrEdit?travelid="
											+ selected.travelid;
									art.dialog.open(link, {
										height : 420,
										width : 1050,
						...
						success : function(data) {
							if (data != null) {
								if ((data.company != null
									...
									if (data.Travelpersonnel == "0") {
										alert("至少保存一行出差人员明细！");
									} else {
										if (data.Traveladdress == "0") {
											alert("至少保存一行出差行程明细！");
										} else {
											if (data.status == "0") {
												jQuery
								...
								if (data.status == "0" || data.status == "3") {
									if (confirm("数据删除后不能恢复，确定删除吗？")) {
										jQuery
												.ajax({
													type : "POST",
													url : '${contextPath}/mx/oa/stravel/delete?travelid='
															+ selected.travelid,
													dataType : 'json',
													error : function(data) {
														alert('服务器处理错误！');
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp;<span
					class="x_content_title">出差申请清单</span> <a href="#"
		...
		<form id="searchForm" name="searchForm" method="post">
			<table class="easyui-form">
				<tbody>
					<tr>
						<td>事由：</td>
						<td><input id="contentLike" name="contentLike"
							class="easyui-validatebox" type="text" value="" /></td>
					</tr>
					<tr>
						<td>申请日期：</td>
