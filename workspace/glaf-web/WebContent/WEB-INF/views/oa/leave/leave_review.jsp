...
<script type="text/javascript">
	...
	function passData() {
		if (confirm("确认审核通过吗？")) {
			...
			jQuery.ajax({
				type : "POST",
				url : '${contextPath}/mx/oa/leave/submitData',
				data : params,
				dataType : 'json',
				error : function(data) {
					alert('服务器处理错误！');
	...
	function noPassData() {
		...
		if (confirm("确认审核不通过吗?")) {
			...
			jQuery.ajax({
				type : "POST",
				url : '${contextPath}/mx/oa/leave/rollbackData',
				data : params,
				dataType : 'json',
				error : function(data) {
					alert('服务器处理错误！');
	...
	<div class="easyui-layout" data-options="fit:true">
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<span class="x_content_title" id="titleSpan">请假申请审核</span> <a
		...
		<div data-options="region:'center',border:false,cache:true">
			<form id="iForm" name="iForm" method="post">
				<input type="hidden" id="leaveid" name="leaveid"
					value="${leave.leaveid}" /> <input type="hidden"
					id="processInstanceId" name="processInstanceId"
					value="${leave.processinstanceid}" />
					...
					<tbody>
						<tr>
							...
							<td align="left"><input id="area" class="easyui-combobox"
								name="area" value="${leave.area}" disabled="disabled"
							...
							<td align="left"><input id="company" class="easyui-combobox"
								name="company" value="${leave.company}" disabled="disabled"
							...
							<td align="left"><input id="dept" class="easyui-combobox"
								name="dept" value="${leave.dept}" disabled="disabled"
						...
						<tr>
							...
							<td align="left"><input id="appuser" class="easyui-combobox"
								name="appuser" value="${leave.appuser}" disabled="disabled"
						...
						<tr>
							<td width="20%" align="left">请假类型：</td>
							<td align="left"><input id="type" class="easyui-combobox"
								name="type" value="${leave.type}" disabled="disabled"
								data-options="required:true,valueField:'code',textField:'name',url:'${contextPath}/rs/dictory/jsonArray/QJLX'" />
							</td>
						</tr>
						<tr>
							...
							<td align="left" colspan="5"><textarea id="content"
									...
									disabled="disabled" class="easyui-validatebox">${leave.content}</textarea>
							</td>
						</tr>
						<tr>
							...
							<td align="left" colspan="5"><input id="startdate"
								...
								value="${leave.starttime}" />时起至 <input id="enddate"
								...
								class="easyui-numberbox" min="0" max="24" style="width: 25px"
								data-options="required:true" disabled="disabled"
								value="${leave.endtime}" />时止共<input id="leavesum"
								name="leavesum" type="text" data-options="required:true"
						...
						<tr>
							...
							<td align="left" colspan="5"><a
								href="javascript:uploadFile(13, ${leave.leaveid}, 0)">附件上传</a> <jsp:include
									page="/others/attachment/showCount">
									<jsp:param name="referType" value="13" />
									<jsp:param name="referId" value="${leave.leaveid}" />
								</jsp:include></td>
						</tr>
						<tr>
