...
<script type="text/javascript">
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '请假申请审核查询');
	...
	function passData() {
		...
		if (ids.length > 0) {
			if (confirm("确认审核通过吗？")) {
				...
				jQuery.ajax({
					type : "POST",
					url : '${contextPath}/mx/oa/leave/submitData?leaveids='
							+ ids,
					data : params,
					dataType : 'json',
					error : function(data) {
	...
	function editSelected() {
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected) {
			var link = "${contextPath}/mx/oa/leave/review?leaveid="
					+ selected.leaveid;
			art.dialog.open(link, {
				height : 420,
				width : 950,
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp; <span
					class="x_content_title">请假申请审核列表</span>
				...
				<tbody>
					...
					<tr>
						...
						<td><input id="type" class="easyui-combobox" name="type"
							value=""
							data-options="valueField:'code',textField:'name',url:'${contextPath}/rs/dictory/jsonArray/QJLX'" /></td>
					</tr>
					<tr>
						<td>申请日期：</td>
