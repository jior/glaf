				...
				.datagrid(
						{
							...
							striped : true,
							collapsible : true,
							url : '${contextPath}/mx/oa/overtime/json?rstatus='
									+ (
...
<%=request.getParameter("status")%>
	...
	function addNew() {
		var link = "${contextPath}/mx/oa/overtime/addOrEdit";
		art.dialog.open(link, {
			height : 420,
			width : 850,
			title : "添加记录",
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '加班申请列表查询');
						...
						success : function(data) {
							if (data != null) {
								if (data.status == "0" || data.status == "3") {
									var link = "${contextPath}/mx/oa/overtime/addOrEdit?id="
											+ selected.id;
									art.dialog.open(link, {
										height : 420,
										width : 850,
								...
								if (data.status == "0" || data.status == "3") {
									if (confirm("数据删除后不能恢复，确定删除吗？")) {
										jQuery
												.ajax({
													type : "POST",
													url : '${contextPath}/mx/oa/overtime/delete?id='
															+ selected.id,
													dataType : 'json',
													error : function(data) {
														alert('服务器处理错误！');
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp;<span
					class="x_content_title">加班申请清单</span> <a href="#"
