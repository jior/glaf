...
<script type="text/javascript">
	...
	function editSelected() {
		...
		if (selected) {
			...
			var link = "${contextPath}/mx/oa/budget/review?budgetid="
					+ selected.budgetid;
		...
		if (ids.length > 0) {
			if (confirm("确认审核通过吗？")) {
				...
				jQuery
						.ajax({
							type : "POST",
							url : '${contextPath}/mx/oa/budget/submit?budgetids='
									+ ids,
							dataType : 'json',
							error : function(data) {
								alert('服务器处理错误！');
	...
	function noPassData() {
		...
		if (ids.length > 0) {
			...
			if (confirm("确认审核不通过吗？")) {
				...
				jQuery.ajax({
					type : "POST",
					url : '${contextPath}/mx/oa/budget/rollback?budgetids='
							+ ids + '&option='
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp; <span
					class="x_content_title">支付预算审批列表</span>
