				...
				.datagrid(
						{
							...
							striped : true,
							collapsible : true,
							url : '${contextPath}/mx/oa/ltravel/json?rstatus='
									+ (
...
<%=request.getParameter("status")%>
	...
	function addNew() {
		var link = "${contextPath}/mx/oa/ltravel/addOrEdit";
		art.dialog.open(link, {
			height : 420,
			width : 850,
			title : "添加记录",
						...
						success : function(data) {
							if (data != null) {
								if (data.status == "0" || data.status == "3") {
									var link = "${contextPath}/mx/oa/ltravel/addOrEdit?travelid="
											+ selected.travelid;
									art.dialog.open(link, {
										height : 420,
										width : 850,
								...
								if (data.status == "0" || data.status == "3") {
									if (confirm("数据删除后不能恢复，确定删除吗？")) {
										jQuery
												.ajax({
													type : "POST",
													url : '${contextPath}/mx/oa/ltravel/delete?travelid='
															+ selected.travelid,
													dataType : 'json',
													error : function(data) {
														alert('服务器处理错误！');
