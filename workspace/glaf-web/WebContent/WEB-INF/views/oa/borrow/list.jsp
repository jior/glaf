...
<script type="text/javascript">
	...
	function submit() {
		...
		if (ids.length != 1) {
			...
			if (confirm("确定提交吗？")) {
				var borrowid = params.borrowid;
				jQuery.ajax({
					type : "POST",
					url : '${contextPath}/mx/oa/borrow/submit2?borrowid='
							+ borrowid,
					data : params,
					dataType : 'json',
					async : false,
				...
				.datagrid(
						{
							...
							striped : true,
							collapsible : true,
							url : '${contextPath}/mx/oa/borrow/json?rstatus='
									+ rstatus,
							sortName : 'id',
							sortOrder : 'desc',
							remoteSort : false,
	...
	function addNew() {
		//location.href="${contextPath}/mx/oa/borrow/init";
		var link = "${contextPath}/mx/oa/borrow/init";
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '员工借支申请列表查询');
	...
	function editSelected() {
		...
		if (selected) {
			//	location.href="${contextPath}/mx/oa/borrow/init&borrowid="+selected.id;
			var link = "${contextPath}/mx/oa/borrow/init?borrowid="
					+ selected.id;
			art.dialog.open(link, {
				height : 420,
				width : 720,
	...
	function deleteSelections() {
		...
		if (ids.length > 0) {
			if (confirm("数据删除后不能恢复，确定删除吗？")) {
				var borrowids = ids.join(',');
				jQuery.ajax({
					type : "POST",
					url : '${contextPath}/mx/oa/borrow/delete?borrowids='
							+ borrowids,
					dataType : 'json',
					error : function(data) {
						alert('服务器处理错误！');
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp;<span
					class="x_content_title">员工借支申请列表</span> <a href="#"
		...
		<form id="searchForm" name="searchForm" method="post">
			<table class="easyui-form">
				<tbody>
					<tr>
						<td>编号</td>
						<td><input id="borrowNoLike" name="borrowNoLike" size="12"
