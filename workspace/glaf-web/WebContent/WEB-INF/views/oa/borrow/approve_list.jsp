...
<script type="text/javascript">
				...
				.datagrid(
						{
							...
							striped : true,
							collapsible : true,
							url : '${contextPath}/mx/oa/borrowApprove/json',
							sortName : 'id',
							sortOrder : 'desc',
							remoteSort : false,
							singleSelect : false,
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '员工借支申请审核查询');
	...
	function approveWin() {
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected) {
			var link = "${contextPath}/mx/oa/borrowApprove/approve?borrowids="
					+ selected.id;
			// location.href= link ;
			art.dialog.open(link, {
				height : 420,
		...
		if (ids.length > 0) {
			if (confirm("确认审核通过吗？")) {
				...
				jQuery
						.ajax({
							type : "POST",
							url : '${contextPath}/mx/oa/borrowApprove/approveData?borrowids='
									+ borrowids + '&isAgree=isAgree',
							dataType : 'json',
							data : params,
							error : function(data) {
								alert('服务器处理错误！');
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp;<span
					class="x_content_title">员工借支申请审核列表</span> <a href="#"
