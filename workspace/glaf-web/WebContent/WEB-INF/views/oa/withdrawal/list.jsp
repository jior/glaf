...
<script type="text/javascript">
   ...
   var rstatus = '<%=request.getParameter("status")%>';
	...
	//新增
	function addNew() {
		var link = "${contextPath}/mx/oa/withdrawal/edit";
	...
	function searchWin() {
		jQuery('#dlg').dialog('open').dialog('setTitle', '取现申请列表查询');
	}
	...
	function editSelected() {
		...
		if (selected) {
			var link = "${contextPath}/mx/oa/withdrawal/edit?withdrawalid="
					+ selected.id;
			art.dialog.open(link, {
				height : 420,
				width : 720,
		...
		if (ids.length != 1) {
			...
			if (confirm("确定提交吗？")) {
				...
				jQuery
						.ajax({
							type : "POST",
							url : '${contextPath}/mx/oa/withdrawal/submit?withdrawalids='
									+ withdrawalid,
							data : params,
							dataType : 'json',
							async : false,
		...
		if (ids.length > 0) {
			if (confirm("数据删除后不能恢复，确定删除吗？")) {
				...
				jQuery
						.ajax({
							type : "POST",
							url : '${contextPath}/mx/oa/withdrawal/delete?withdrawalids='
									+ withdrawalids,
							dataType : 'json',
							error : function(data) {
								alert('服务器处理错误！');
		...
		<div data-options="region:'north',split:true,border:true"
			...
			<div class="toolbar-backgroud">
				<img src="${contextPath}/images/window.png"> &nbsp;<span
					class="x_content_title">取现申请列表</span> <a href="#"
