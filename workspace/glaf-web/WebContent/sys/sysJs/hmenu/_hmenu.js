...
DynarchMenu.prototype.createMenuItem = function (li, parent, horiz, arrow) {
	...
	info = new DynarchMenu.MenuItem({html_popup:html_popup, separator:html_popup || !/\S/.test(label) && !icon, icon:icon, label:label, parent:parent, submenu:null, tooltip:tooltip, action:action, menu:this, disabled:disabled, nohover:nohover, align:licl ? (DynarchMenu._RE_CTX_AL.test(licl) ? RegExp.$1 : "mouse") : "mouse"});
	if (li.id) {
	...
	if (DynarchMenu._RE_CTX_ID.test(licl)) {
		...
		if (DynarchMenu._RE_CTX_CL.test(licl)) {
...
DynarchMenu._documentMouseOver = function (ev) {
	...
	if (menu && menu.config.electric) {
		...
		if (!el || el === document.body) {
			...
			if (!_dynarch_top.DynarchMenu._T) {
				_dynarch_top.DynarchMenu._T = _dynarch_top.setTimeout("_dynarch_top.DynarchMenu._closeOtherMenus(null); _dynarch_top.DynarchMenu._T = null;", tmout);
			}
		} else {
...
DynarchMenu.prototype.createMenuTree = function (ul, horiz) {
	...
	info = new DynarchMenu.MenuTree({menu:this, base:base, horiz:horiz, element:ret, active_submenu:null, active_item:null, visible:false, keymap:{}, config:cfg, _T_close:null, open_left:DynarchMenu._RE_OPL.test(ul.className)});
	if (ul.id) {
		info.id = ul.id;
	}
...
DynarchMenu._nfo = {product:"hmenu-2.9", licensee:"Dynarch.com user: wan1976", license_key:"linkware-7603066", purchase_date:"Mon Jan  8 15:13:11 2007 GMT", license_type:"linkware"};
