...
public class CsvHelper {
	...
	private ArrayList<String> errorLineNoList = new ArrayList<String>();// CSV错误行号
	...
	private Long defaultReadRecordCount = new Long(10000);//分段读取数据时默认的一次读取数据条数为1万行
	...
	/**
	 * 按默认的获取条数（1万条）获取数据到bodylist中
	 * 获取前将清空bodylist之前已获取到的数据
	 * 即bodylist中只保留最近一次获取到的数据。
	 * 
	 * @return 获取数据成功状态

	 */
	public boolean getMoreData() throws Exception {
		this.setRdWrFalg(this.CsvRd_Flag);
		this.checkInitRdorWr();
		return this.getCsvData(defaultReadRecordCount);
	}
	
	/**
	 * 按传入的获取条数获取数据到bodylist中
	 * 获取前将清空bodylist之前已获取到的数据
	 * 即bodylist中只保留最近一次获取到的数据。
	 * 
	 * @return 获取数据成功状态
	 ...
	 */
	public boolean getMoreData(Long nextRecordsCount) throws Exception {
		this.setRdWrFalg(this.CsvRd_Flag);
		this.checkInitRdorWr();
		return this.getCsvData(nextRecordsCount);
	}
	...
	private boolean getCsvData(Long recordsCount) {
		if(null == recordsCount || 0 == recordsCount.longValue()){
			throw new BatchException("recordsCount 不能为null");
		}
		boolean rtTf = true;
		String rtMessage = "";
		messageList = new ArrayList<String>();
		csvHeaderList = new ArrayList<String[]>();
		if(null == csvBodyList || null != recordsCount){
			csvBodyList = new ArrayList<String[]>();
		}
		int count = 0;
		try {
			...
			//读取文件header
			if(this.isExistHeader == true && -1 == reader.getCurrentRecord()){
				csvHeaderList = new ArrayList<String[]>();
				if(reader.readRecord()){
					csvHeaderList.add(reader.getValues());
				}
			}
			
			//读取文件内容
			while (reader.readRecord()) { // 逐行读入数据
				csvBodyList.add(reader.getValues());
				count++;
	...
	private boolean checkCsvData() {
		...
		boolean rtTf = true;
		messageList = new ArrayList<String>();
		errorLineNoList = new ArrayList<String>();
		try {
			...
			for (int i = 0; i < csvBodyList.size(); i++) {
				...
				if (dataStrs.length != this.columnsCount
					...
					errorLineNoList.add(String.valueOf(i));
					messageList.add(rtMessage);
					rtTf = false;
	...
	public ArrayList<String> getErrorLineNoList() {
		return errorLineNoList;
	}
