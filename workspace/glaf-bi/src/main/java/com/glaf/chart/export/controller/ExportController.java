...
public class ExportController extends HttpServlet {
	...
	private static Float scaleToFloat(String scale)
		...
		if (scale != null) {
			try {
				...
				logger.error("Parameter scale is wrong for value: " + scale,
						nfe.fillInStackTrace());
				throw new SVGConverterException(
						"Parameter scale is wrong for value: " + scale);
			}
		}
		return null;
	}
	...
	private static Float widthToFloat(String width)
		...
		if (width != null) {
			...
			try {
				...
				logger.error("Parameter width is wrong for value: " + width,
						nfe.fillInStackTrace());
				throw new SVGConverterException(
						"Parameter width is wrong for value: " + width);
			}
	...
	public SVGConverter getSvgConverter() {
		if (svgConverter == null) {
			svgConverter = ContextFactory.getBean("converter");
		}
		return svgConverter;
	}

	private String convert(String svg, MimeType mime, String width,
			String scale, String options, String constructor, String callback,
			String globalOptions, String filename)
			throws SVGConverterException, PoolException,
			NoSuchElementException, TimeoutException, ServletException {
		...
		if (convertSvg && mime.equals(MimeType.SVG)) {
			output = getSvgConverter().redirectSVG(input, filename);
		} else {
			output = getSvgConverter()
					.convert(input, mime, constructor, callback, globalOptions,
							parsedWidth, parsedScale, filename);
		}
	...
	public HttpEntity<byte[]> exporter(
			...
			SVGConverterException, NoSuchElementException, PoolException,
			TimeoutException, IOException, ZeroRequestParameterException {
		...
		if ("GET".equalsIgnoreCase(request.getMethod())) {
			...
			// Handle redirect downloads for Android devices, these come in
		...
		// check for visitors who don't know this domain is really only for the
		// exporting service ;)
		...
		/*
		 * Most JSONP implementations use the 'callback' request parameter and
		 * this overwrites the original callback parameter for chart creation
		 * with Highcharts. If JSONP is used we recommend using the
		 * requestparameter callbackHC as the callback for Highcharts. store the
		 * callback method name and reset the callback parameter, otherwise it
		 * will be used when creation charts
		 */
		if (jsonp) {
			async = true;
		...
		/*
		 * If randomFilename is not null, then we want to save the filename in
		 * session, in case of GET is used later on
		 */
		if (isAndroid) {
			logger.debug("storing randomfile in session: "
					+ FilenameUtils.getName(randomFilename));
			session.setAttribute("tempFile",
					FilenameUtils.getName(randomFilename));
		}

		String output = convert(svg, mime, width, scale, options, constructor,
				callback, globalOptions, randomFilename);
	...
	public HttpEntity<byte[]> exportFromJson(@PathVariable("name") String name,
			...
			@RequestBody String requestBody) throws SVGConverterException,
		...
		if (MimeType.PDF.equals(mime)) {
			...
			json = new StringBuilder(json).replace(ind, ind + 1,
					",\"outfile\":\"" + randomFilename).toString()
					+ "\"}";
		}

		String result = getSvgConverter().requestServer(json);
		ByteArrayOutputStream stream;
	...
	public HttpEntity<byte[]> getFile(@PathVariable("name") String name,
			@PathVariable("ext") String extension)
			throws SVGConverterException, IOException {

		Path path = Paths.get(TempDir.getOutputDir().toString(), name + "."
				+ extension);
	...
	public ModelAndView handleIllegalStateException(Exception ex,
			HttpServletResponse response) {
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("error");
		String message = "Something went wrong while converting. "
	...
	public ModelAndView handleZeroRequestParameterException(Exception ex,
			HttpServletResponse response) {
		ModelAndView modelAndView = new ModelAndView();
	...
	private ByteArrayOutputStream outputToStream(String output, boolean base64)
			throws SVGConverterException {
	...
	private ByteArrayOutputStream writeFileToStream(String filename)
			throws SVGConverterException {
		ByteArrayOutputStream stream = new ByteArrayOutputStream();
		...
		try {
			...
			logger.error("Tried to read file from filesystem: "
					+ ioex.getMessage());
			throw new SVGConverterException(
					"IOException: cannot find your file to download...");
		}
