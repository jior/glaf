package com.glaf.chart.export.component;
...
public class MonitorComponent {
	...
	public String report() {
		...
		if (elapsedMinutes > 60) {
			report = String
					.format("##### HOURLY REPORT request count: %d Error count: %d #####",
							this.getCount(), this.getCountError());
			// resetting
			count.set(0);
			error.set(0);
			...
			report = String
					.format("request count: %d, Error count: %d, Elapsed time (min): %d, Rate: %d",
							this.getCount(), this.getCountError(),
							elapsedMinutes, calculateRatePerMinute());
		}
