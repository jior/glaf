...
public class ${entityName}BaseController {
	...
	public byte[] json(HttpServletRequest request,
		...
		int pageNo = ParamUtil.getInt(paramMap, "page");
		int limit = ParamUtil.getInt(paramMap, "rows");
	    ...
	    String sortorder  = ParamUtil.getString(paramMap, "order");
		...
		// 当前页数设置
		pageInfo.put("page", Integer.valueOf(pageNo));

		// 每页记录数
		pageInfo.put("limit", Integer.valueOf(limit));

		// 总数据量设置
		pageInfo.put("total", Integer.valueOf(jpage
				.getTotalRecordCount()));
