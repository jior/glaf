...
public class CompressionCodecFactory {
	...
	public static final Log LOG = LogFactory
			.getLog(CompressionCodecFactory.class.getName());

	/**
	 * A map from the reversed filename suffixes to the codecs. This is probably
	 * overkill, because the maps should be small, but it automatically supports
	 * finding the longest matching suffix.
	 */
	...
	public String toString() {
		...
		Iterator<Map.Entry<String, CompressionCodec>> itr = codecs.entrySet()
				.iterator();
		buf.append("{ ");
		if (itr.hasNext()) {
	 ...
	 * @return a list of the Configuration classes or null if the attribute was
	 *         not set
	 */
	...
	/**
	 * Find the codecs specified in the config value io.compression.codecs and
	 * register them. Defaults to gzip and zip.
	 */
	public CompressionCodecFactory(Configuration conf) {
		...
		if (codecClasses == null) {
			...
			Iterator<Class<? extends CompressionCodec>> itr = codecClasses
					.iterator();
			while (itr.hasNext()) {
				CompressionCodec codec = ReflectUtils.newInstance(itr.next(),
						conf);
				addCodec(codec);
			}
		}
	...
	public CompressionCodec getCodec(File file) {
		...
		if (codecs != null) {
			...
			String reversedFilename = new StringBuffer(filename).reverse()
					.toString();
			SortedMap<String, CompressionCodec> subMap = codecs
					.headMap(reversedFilename);
			if (!subMap.isEmpty()) {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < args.length; ++i) {
			if ("-in".equals(args[i])) {
				...
				CompressionCodec codec = factory.getCodec(new File(args[i]));
				if (codec == null) {
					System.out.println("Codec for " + args[i] + " not found.");
				} else {
					if (encode) {
						CompressionOutputStream out = codec
								.createOutputStream(new java.io.FileOutputStream(
										args[i]));
						...
						CompressionInputStream in = codec
								.createInputStream(new java.io.FileInputStream(
										args[i]));
						byte[] buffer = new byte[100];
						int len = in.read(buffer);
						while (len > 0) {
