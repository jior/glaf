...
import com.glaf.core.io.compress.Compressor;
import com.glaf.core.util.NativeCodeLoader;

/**
 * A {@link Compressor} based on the popular zlib compression algorithm.
 * http://www.zlib.net/
 * 
 */
public class ZlibCompressor implements Compressor {
	protected static final Log LOG = LogFactory.getLog(ZlibCompressor.class);
	...
	public static enum CompressionStrategy {
		...
		/**
		 * Compression strategy to limit match distances to one (run-length
		 * encoding).
		 */
		RLE(3),
	...
	protected final void construct(CompressionLevel level,
			CompressionStrategy strategy, CompressionHeader header,
			int directBufferSize) {
	}

	/**
	 * Creates a new compressor with the default compression level. Compressed
	 * data will be generated in ZLIB format.
	 */
	public ZlibCompressor() {
		this(CompressionLevel.DEFAULT_COMPRESSION,
				CompressionStrategy.DEFAULT_STRATEGY,
				CompressionHeader.DEFAULT_HEADER, DEFAULT_DIRECT_BUFFER_SIZE);
	}
	...
	public ZlibCompressor(Configuration conf) {
		this(ZlibFactory.getCompressionLevel(conf), ZlibFactory
				.getCompressionStrategy(conf),
				CompressionHeader.DEFAULT_HEADER, DEFAULT_DIRECT_BUFFER_SIZE);
	}
	 ...
	 * @param header
	 *            Compression header #CompressionHeader
	 ...
	 * in the given Configuration. It will reset the compressor's compression
	 * level and compression strategy.
	 * 
	 * @param conf
	...
	public synchronized void reinit(Configuration conf) {
		...
		stream = init(level.compressionLevel(), strategy.compressionStrategy(),
	 ...
	 * 
	 * @return the total (non-negative) number of uncompressed bytes input so
	 *         far
	 */
