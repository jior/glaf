...
package com.glaf.core.io.compress.zlib;
...
import com.glaf.core.io.compress.Decompressor;
import com.glaf.core.io.compress.DoNotPool;
...
public class BuiltInGzipDecompressor implements Decompressor {
	...
	private static enum GzipStateLabel {
		/**
		 * Immediately prior to or (strictly) within the 10-byte basic gzip
		 * header.
		 */
		HEADER_BASIC,
		/**
		...
		HEADER_CRC,
		/**
		 * Immediately prior to or within the main compressed (deflate) data
		 * stream.
		 */
		DEFLATE_STREAM,
		/**
		 * Immediately prior to or (strictly) within the 4-byte uncompressed
		 * CRC.
		 */
		TRAILER_CRC,
		/**
	...
	/** {@inheritDoc} */
	/*
	 * In our case, the input data includes both gzip header/trailer bytes
	 * (which we handle in executeState()) and deflate-stream bytes (which we
	 * hand off to Inflater).
	 * 
	 * NOTE: This code assumes the data passed in via b[] remains unmodified
	 * until _we_ signal that it's safe to modify it (via needsInput()). The
	 * alternative would require an additional buffer-copy even for the bulk
	 * deflate stream, which is a performance hit we don't want to absorb.
	 * (Decompressor now documents this requirement.)
	 */
	 ...
	 */
	/*
	 * From the caller's perspective, this is where the state machine lives. The
	 * code is written such that we never return from decompress() with data
	 * remaining in userBuf unless we're in FINISHED state and there was data
	 * beyond the current gzip member (e.g., we're within a concatenated gzip
	 * stream). If this ever changes, {@link #needsInput()} will also need to be
	 * modified (i.e., uncomment the userBufLen condition).
	 * 
	 * The actual deflate-stream processing (decompression) is handled by Java's
	 * Inflater class. Unlike the gzip header/trailer code (execute* methods
	 * below), the deflate stream is never copied; Inflater operates directly on
	 * the user's buffer.
	 */
	public synchronized int decompress(byte[] b, int off, int len)
		...
		if (state == GzipStateLabel.DEFLATE_STREAM) {
			// hand off user data (or what's left of it) to Inflater--but note
			// that
			// Inflater may not have consumed all of previous bufferload (e.g.,
			// if
			// data highly compressed or output buffer very small), in which
			// case
			...
			crc.update(b, off, numAvailBytes); // CRC-32 is on _uncompressed_
												// data
			if (inflater.finished()) {
				...
				// could save a copy of userBufLen at call to
				// inflater.setInput() and
				// verify that bytesRemaining <= origUserBufLen, but would have
				// to
				// be a (class) member variable...seems excessive for a sanity
				// check
				userBufOff -= bytesRemaining;
				userBufLen = bytesRemaining; // or "+=", but guaranteed 0 coming
												// in
			} else {
	...
	/**
	 * Parse the gzip header (assuming we're in the appropriate state). In order
	 * to deal with degenerate cases (e.g., user buffer is one byte long), we
	 * copy (some) header bytes to another buffer. (Filename, comment, and
	 * extra-field bytes are simply skipped.)</p>
	 * 
	 * See http://www.ietf.org/rfc/rfc1952.txt for the gzip spec. Note that no
	 * version of gzip to date (at least through 1.4.0, 2010-01-20) supports the
	 * FHCRC header-CRC16 flagbit; instead, the implementation treats it as a
	 * multi-file continuation flag (which it also doesn't support). :-( Sun's
	 * JDK v6 (1.6) supports the header CRC, however, and so do we.
	 */
	private void executeHeaderState() throws IOException {
		...
		if (state == GzipStateLabel.HEADER_BASIC) {
			int n = Math.min(userBufLen, 10 - localBufOff); // (or
															// 10-headerBytesRead)
		...
		if (state == GzipStateLabel.HEADER_EXTRA_FIELD) {
			if (hasExtraField) {
				// 2 substates: waiting for 2 bytes => get
				// numExtraFieldBytesRemaining,
				// or already have 2 bytes & waiting to finish skipping
				// specified length
		...
		if (state == GzipStateLabel.HEADER_FILENAME) {
			if (hasFilename) {
				...
				if (!doneWithFilename) {
					return; // exit early: used up entire buffer without hitting
							// NULL
				}
			}
		...
		if (state == GzipStateLabel.HEADER_CRC) {
			if (hasHeaderCRC) {
				...
				state = GzipStateLabel.DEFLATE_STREAM; // switching to Inflater
														// now
			}
		}
	}
	...
	/**
	 * Parse the gzip trailer (assuming we're in the appropriate state). In
	 * order to deal with degenerate cases (e.g., user buffer is one byte long),
	 * we copy trailer bytes (all 8 of 'em) to a local buffer.</p>
	 * 
	...
	private void executeTrailerState() throws IOException {
		...
		if (state == GzipStateLabel.TRAILER_CRC) {
			// localBuf was empty before we handed off to Inflater, so we handle
			// this
			// exactly like header fields
			assert (localBufOff < 4); // initially 0, but may need multiple
										// calls
	...
	/**
	 * Returns the total number of compressed bytes input so far, including gzip
	 * header/trailer bytes.</p>
	 * 
	 ...
	 * {@link #setInput(byte[] b, int off, int len)} and before
	 * {@link #decompress(byte[] b, int off, int len)}. (That is, after
	 * {@link #decompress(byte[] b, int off, int len)} it always returns zero,
	 * except in finished state with concatenated data.)</p>
	 * 
	...
	public synchronized void reset() {
		// could optionally emit INFO message if state !=
	 ...
	 */
	/*
	 * Flag bits (remainder are reserved and must be zero): bit 0 FTEXT bit 1
	 * FHCRC (never implemented in gzip, at least through version 1.4.0; instead
	 * interpreted as "continuation of multi- part gzip file," which is
	 * unsupported through 1.4.0) bit 2 FEXTRA bit 3 FNAME bit 4 FCOMMENT [bit 5
	 * encrypted]
	 */
	private void processBasicHeader() throws IOException {
		...
		if (readUByte(localBuf, 2) != GZIP_DEFLATE_METHOD) {
			throw new IOException(
					"gzip data not compressed with deflate method");
		}
	...
	private long readUIntLE(byte[] b, int off) {
		return ((((long) (b[off + 3] & 0xff) << 24)
				...
				| ((long) (b[off + 1] & 0xff) << 8) | ((long) (b[off] & 0xff))) & 0xffffffff);
	}
