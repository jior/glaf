...
import com.glaf.core.util.threads.StringManager;

/**
 * Same as a java.util.concurrent.ThreadPoolExecutor but implements a much more
 * efficient {@link #getSubmittedCount()} method, to be used to properly handle
 * the work queue. If a RejectedExecutionHandler is not specified a default one
 * will be configured and that one will always throw a
...
public class MxThreadPoolExecutor extends
	...
	protected boolean currentThreadShouldBeStopped() {
		if (threadRenewalDelay >= 0
			...
			if (currentTaskThread.getCreationTime() < this.lastContextStoppedTime
					.longValue()) {
				return true;
			}
		}
	...
	public void contextStopping() {
		...
		TaskQueue taskQueue = getQueue() instanceof TaskQueue ? (TaskQueue) getQueue()
				: null;
		if (taskQueue != null) {
