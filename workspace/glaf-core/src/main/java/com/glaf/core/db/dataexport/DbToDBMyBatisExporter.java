...
public class DbToDBMyBatisExporter {
	...
	public void exportTable(String systemName, String dbtype, String dataDir,
			String tableName) {
		...
		if (StringUtils.equals(dbtype, "h2")) {
			...
			props.setProperty("jdbc.url", "jdbc:h2:" + dataDir);
			...
			props.setProperty("jdbc.url", "jdbc:sqlite:" + dataDir);
			props.setProperty("jdbc.user", "sa");
			props.setProperty("jdbc.password", "");
			DBConfiguration.addDataSourceProperties(jdbc_name, props);
		}
	...
	public void exportTables(String systemName, String dbtype, String dataDir) {
		...
		if (!tables.isEmpty()) {
			for (String tableName : tables) {
				...
				while (retry < 3 && !success) {
					try {
						retry++;
						this.exportTable(systemName, dbtype, dataDir, tableName);
						success = true;
					} catch (Exception ex) {
						ex.printStackTrace();
					}
