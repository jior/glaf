...
import com.glaf.core.io.compress.DefaultCodec;
import com.glaf.core.io.compress.zlib.*;
...
public class GzipCodec extends DefaultCodec {
	...
	public CompressionOutputStream createOutputStream(OutputStream out)
			throws IOException {
		return (ZlibFactory.isNativeZlibLoaded(conf)) ? new CompressorStream(
				out, createCompressor(), conf.getInt("io.file.buffer.size",
	...
	public Compressor createCompressor() {
		return (ZlibFactory.isNativeZlibLoaded(conf)) ? new GzipZlibCompressor(
				conf) : null;
	}

	public Class<? extends Compressor> getCompressorType() {
		return ZlibFactory.isNativeZlibLoaded(conf) ? GzipZlibCompressor.class
				: null;
	}
	...
	public CompressionInputStream createInputStream(InputStream in,
		...
		return new DecompressorStream(in, decompressor, conf.getInt(
				"io.file.buffer.size", 4 * 1024));
	}

	public Decompressor createDecompressor() {
		return (ZlibFactory.isNativeZlibLoaded(conf)) ? new GzipZlibDecompressor()
				: new BuiltInGzipDecompressor();
	}

	public Class<? extends Decompressor> getDecompressorType() {
		return ZlibFactory.isNativeZlibLoaded(conf) ? GzipZlibDecompressor.class
	...
	static final class GzipZlibCompressor extends ZlibCompressor {
		...
		public GzipZlibCompressor(Configuration conf) {
			super(ZlibFactory.getCompressionLevel(conf), ZlibFactory
