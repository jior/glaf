...
public class BuiltInZlibDeflater extends Deflater implements Compressor {
	...
	BuiltInZlibDeflater(Configuration conf) {
		this(null == conf ? DEFAULT_COMPRESSION : ZlibFactory
				.getCompressionLevel(conf).compressionLevel());
		if (conf != null) {
			final ZlibCompressor.CompressionStrategy strategy = ZlibFactory
	 ...
	 * DEFAULT_STRATEGY. It will use DEFAULT_STRATEGY as default if the
	 * configured compression strategy is not supported.
	 */
	@Override
