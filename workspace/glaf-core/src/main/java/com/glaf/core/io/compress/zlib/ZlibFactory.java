...
import com.glaf.core.io.compress.zlib.ZlibCompressor.CompressionLevel;
import com.glaf.core.io.compress.zlib.ZlibCompressor.CompressionStrategy;
import com.glaf.core.util.NativeCodeLoader;

/**
 * A collection of factories to create the right zlib/gzip
...
public class ZlibFactory {
	...
	/**
	 * Check if native-zlib code is loaded & initialized correctly and can be
	 * loaded for this job.
	 * 
	 * @param conf
	 *            configuration
	 * @return <code>true</code> if native-zlib is loaded & initialized and can
	 ...
	 * @return the appropriate type of the zlib compressor.
	 */
	public static Class<? extends Compressor> getZlibCompressorType(
			Configuration conf) {
		return (isNativeZlibLoaded(conf)) ? ZlibCompressor.class
	 ...
	 * @return the appropriate type of the zlib decompressor.
	 */
	public static Class<? extends Decompressor> getZlibDecompressorType(
			Configuration conf) {
		return (isNativeZlibLoaded(conf)) ? ZlibDecompressor.class
	...
	public static Decompressor getZlibDecompressor(Configuration conf) {
		return (isNativeZlibLoaded(conf)) ? new ZlibDecompressor()
