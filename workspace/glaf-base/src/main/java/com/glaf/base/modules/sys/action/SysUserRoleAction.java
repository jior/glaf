...
public class SysUserRoleAction extends DispatchActionSupport {
	private static final Log logger = LogFactory
			.getLog(SysUserRoleAction.class);
	 ...
	 * @throws Exception
	 */
	public ActionForward showSysAuth(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
	 ...
	 * @throws Exception
	 */
	public ActionForward showUserSysAuth(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		long userId = ParamUtil.getLongParameter(request, "id", 0);
		SysUser user = (SysUser) sysUserService.findById(userId);
	 ...
	 * @throws Exception
	 */
	public ActionForward saveUserSysAuth(ActionMapping mapping,
			ActionForm actionForm, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
			...
			authorStart: for (int i = 0; i < userIds.length; i++) {
				...
				if (!startDate.equals("") && !endDate.equals("")) {
					...
					messages.add(
							...
							new ActionMessage(sysUser.getName() + "["
									+ sysUser.getAccount() + "]"
									+ "sys.author_dateErr"));
