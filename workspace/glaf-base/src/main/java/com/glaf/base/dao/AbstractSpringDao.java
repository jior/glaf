...
public class AbstractSpringDao extends HibernateDaoSupport {
	...
	public PageResult getList(final DetachedCriteria detachedCriteria,
		...
		List<Object> orderEntries = null;
		try {
			orderEntries = (List<Object>) ReflectUtils.getFieldValue(impl,
					"orderEntries");
		...
		try {
			List<Object> innerOrderEntries = (List<Object>) ReflectUtils
					.getFieldValue(impl, "orderEntries");
			Iterator<Object> it = orderEntries.iterator();
			while (it.hasNext()) {
				innerOrderEntries.add((Object) it.next());
			}
		} catch (Exception ex) {
			throw new RuntimeException(ex);
		}
