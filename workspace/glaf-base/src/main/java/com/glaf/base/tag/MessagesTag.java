 ...
 * retrieving the messages from <code>Globals.ERROR_KEY</code>, but if the
...
public class MessagesTag extends BodyTagSupport {
	...
	protected static MessageResources messageResources = MessageResources
			.getMessageResources(Globals.Package + ".LocalStrings");
	 ...
	 * Process a message.
	 */
	private void processMessage(ViewMessage report) throws JspException {
		String msg = null;
	...
	public int doEndTag() throws JspException {
		if (processed && (footer != null) && (footer.length() > 0)) {
			String footerMessage = TagUtils.getInstance().message(pageContext,
					bundle, locale, footer);
