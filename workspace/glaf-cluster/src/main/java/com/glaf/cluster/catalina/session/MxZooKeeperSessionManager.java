...
public class MxZooKeeperSessionManager extends ManagerBase implements Lifecycle {

	private final Log log = LogFactory.getLog(MxZooKeeperSessionManager.class);
	...
	protected MxZooKeeperSessionHandlerValve handlerValve;
	...
	protected static String name = "MxZooKeeperSessionManager";
	...
	public Session createSession(String sessionId) {
		...
		try {
			...
			do {
				...
				path = GROUP_NAME + "/" + sessionId;

			} while (zooKeeperClient.saveDataNode(path, NULL_SESSION) == null);
	...
	public ZooKeeperSession loadSessionFromZooKeeper(String id)
		...
		if (id != null && id.trim().length() > 0) {
			try {
				...
				byte[] data = zooKeeperClient.getNode(path);
	...
	public void remove(Session session, boolean update) {
		...
		try {
			zooKeeperClient.deleteNodeRecursively(path);
	...
	public void save(Session session) {
		try {
			...
			if (sessionIsDirty || currentSessionIsPersisted.get() != true) {
				...
				zooKeeperClient.saveDataNode(path,
	...
	protected synchronized void startInternal() throws LifecycleException {
		...
		for (Valve valve : getContainer().getPipeline().getValves()) {
			if (valve instanceof MxZooKeeperSessionHandlerValve) {
				this.handlerValve = (MxZooKeeperSessionHandlerValve) valve;
				this.handlerValve.setMxZooKeeperSessionManager(this);
		...
		ZooKeeperFactory zooKeeperFactory = new ZooKeeperFactory(
				this.getServers());
		zooKeeperClient = new ZooKeeperClientImpl(zooKeeperFactory);
