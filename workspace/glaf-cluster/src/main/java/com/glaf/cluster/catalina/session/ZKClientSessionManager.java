...
public class ZKClientSessionManager extends ManagerBase implements Lifecycle {

	private final Log log = LogFactory.getLog(ZKClientSessionManager.class);
	...
	protected ZKClientSessionHandlerValve handlerValve;
	...
	protected static String name = "ZKClientSessionManager";
	...
	public Session createSession(String sessionId) {
		...
		try {
			...
			ZKClientUtils.createPath(zkClient, path, NULL_SESSION);
	...
	public ZooKeeperSession loadSessionFromZooKeeper(String id)
		...
		if (id != null && id.trim().length() > 0) {
			try {
				...
				byte[] data = ZKClientUtils.readDataMaybeNull(zkClient, path);
	...
	public void remove(Session session, boolean update) {
		...
		try {
			ZKClientUtils.deletePathRecursive(zkClient, path);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void save(Session session) {
		try {
			...
			if (sessionIsDirty || currentSessionIsPersisted.get() != true) {
				...
				ZKClientUtils.writeData(zkClient, path,
	...
	protected synchronized void startInternal() throws LifecycleException {
		...
		for (Valve valve : getContainer().getPipeline().getValves()) {
			if (valve instanceof ZKClientSessionHandlerValve) {
				this.handlerValve = (ZKClientSessionHandlerValve) valve;
				this.handlerValve.setZKClientSessionManager(this);
		...
		zkClient = new ZkClient(servers, getTimeout());
